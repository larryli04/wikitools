...
def isFile(string):
    ...
    if("[[File:" in string or "[[Image:" in string or "[[image:" in string):
        return True
    else:
        return False
def noFile(string):
...
# print(flink("asdff[[asaad|dadf]]asdf"))
...
def clean(string):
    ...
    if(string[0] == "#"): # early outs
        return string, True
    elif("#REDIRECT" in string):
        return string, True
    elif("#Redirect" in string):
        return string, True
    if(not "[" in string):
    ...
    while(isFile(s) and f==False): # get rid of files (they use square brackets)
        # print("nofile")
        s,f = noFile(s)
        # print(s[:50])
    
    f = False
    while((len(s) != 0 and "{" in s and f==False)): # get rid of curly braces (can have square brackets inside)
        try:
            if(s.index("[")<s.index("{")):
                break
        except:
    ...
    if(not "[" in string): # sometimes there are square brackets in curly not detected aboce
        return string, 69
...
t = '''{{Use American English|date = January 2019}}
{{Short description|Special function defined by an integral}}
[[image:Fresnel Integrals (Unnormalised).svg|250px|thumb|
Plots of {{math|{{color|#b30000|''S''(''x'')}}}} and {{math|{{color|#00b300|''C''(''x'')}}}}. The maximum of {{math|''C''(''x'')}} is about {{val|0.977451424}}. If the integrands of {{mvar|S}} and {{mvar|C}} were defined using {{math|{{sfrac|''Ï€''|2}}''t''&lt;sup&gt;2&lt;/sup&gt;}} instead of {{math|''t''&lt;sup&gt;2&lt;/sup&gt;}}, then the image would be scaled vertically and horizontally (see below).]]

The ''Fresnel integrals'' {{math|''S''(''x'')}} and {{math|''C''(''x'')}} are two [[transcendental function]]s named after [[Augustin-Jean Fresnel]] that are used in [[optics]] and are closely related to the [[error function]] ({{math|erf}}). They arise in the description of [[near and far field|near-field]] [[Fresnel diffraction]] phenomena and are defined through the following [[integral]] representations:
&lt;math display=&quot;block&quot;&gt;S(x) = \int_0^x \sin\left(t^2\right)\,dt, \quad C(x) = \int_0^x \cos\left(t^2\right)\,dt.&lt;/math&gt;

The simultaneous [[parametric equation|parametric plot]] of {{math|''S''(''x'')}} and {{math|''C''(''x'')}} is the [[Euler spiral]] (also known as the Cornu spiral or clothoid).

'''
# print("here", clean(t))
# print("ffd",noParentheses(t))
# print(noFile(t))
